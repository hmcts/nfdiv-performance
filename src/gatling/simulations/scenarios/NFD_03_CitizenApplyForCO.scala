  ...
  val CompleteConditionalOrder =
    ...
    group("NFD03CitCO_050_${userType}CheckYourCOAnswers") {
      ...
      exec(http("Check your CO answers")
        ...
        //check for completed sections (1 - joint with 1 app applied, 2 - joint both apps applied, 3 for sole)
        .check(substring("progress-bar__icon--complete").count.in(1, 2, 3)))
